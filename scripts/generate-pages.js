...
const locales = {
	'zh-cn': {
		...
		eyebrow: '公历 ↔ 藏历 · 1951–2051', tagline: '清楚知道，今天按哪里计算。',
...
<head>
	...
	<link rel="icon" href="/favicon.ico"><link rel="apple-touch-icon" href="/apple-touch-icon.png"><link rel="manifest" href="${prefix}manifest.json?v=20260716-4">
	<title>${s.title}</title>
	...
	<link rel="stylesheet" href="/css/style.css?v=20260716-4">
	<script src="/js/langDetect.js?v=20260716-4"></script>
	<script defer src="/js/i18n-data.js?v=20260716-4"></script><script defer src="/js/i18n.js?v=20260716-4"></script><script defer src="/zangli.js?v=20260716-4"></script><script defer src="/eclipse.js?v=20260716-4"></script><script defer src="/js/app.js?v=20260716-4"></script>
</head>
<body>
	<a class="skip-link" href="#calendarWorkspace">Skip to calendar</a>
	<div class="site-shell">
		<header class="site-header"><a class="brand" href="${prefix}" aria-label="${s.name}"><span class="brand-mark" aria-hidden="true">ལོ</span><span><strong>${s.name}</strong><small>${s.short}</small></span></a><div class="header-actions"><label class="sr-only" for="languageSelect">Language</label><select id="languageSelect" class="quiet-select">${languageOptions}</select><button id="themeButton" class="icon-button" type="button" aria-label="Toggle dark mode"><span aria-hidden="true">◐</span></button><button id="settingsButton" class="text-button" type="button">${s.settings}</button></div></header>
		<main>
			<section class="hero" aria-labelledby="pageTitle"><div class="hero-copy"><p class="eyebrow">${s.eyebrow}</p><h1 id="pageTitle">${s.tagline} ${s.boBeta}</h1><button id="basisButton" class="basis-button" type="button"><span>${s.basis}</span><strong id="basisLabel">—</strong></button><div id="dateDiscrepancy" class="date-notice" hidden></div></div><div id="selectedDay" class="today-card" aria-live="polite"></div><article id="nextSpecialDay" class="next-card" aria-live="polite"><p class="section-kicker">${s.next}</p><div class="skeleton-line"></div></article></section>
			...
			<section class="install-card"><div><p class="section-kicker">${s.install}</p><p>${s.installText}</p></div><div class="button-group"><button id="installButton" class="secondary-button" type="button" hidden>${s.installButton}</button><a class="secondary-button" href="/widget/">${s.widget}</a></div></section>
